...
<?php
 include('inc/dbConn.php');
  ...
  // Fetch image from database
    $query = mysqli_query($conn, "SELECT * FROM carsdetail WHERE approval='approved' AND deleted='no' AND buyersname='no' AND (title='".$_SESSION['searchvalue']."' OR make='".$_SESSION['searchvalue']."' OR model='".$_SESSION['searchvalue']."')");
  ...
  <form method="POST">
  <button type="submit" id="purchase" name="submit36" style="
    ...
    border: none;
    cursor: pointer;
    " onclick="newWindow('buyconfirm.php',500,500)">Purchase</button>
  </form>
